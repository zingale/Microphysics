...
contains
  ...
  subroutine f_rhs(n, t, y, ydot, rpar, ipar)
    ...
    ! (rho h).
    integer :: n
    real(kind=rt) :: y(n), ydot(n)
    real(kind=rt) :: rpar
    integer :: ipar

    real(kind=rt) :: t

    real(kind=rt) :: rho_Hnuc
  ...
  subroutine jac(neq, t, y, ml, mu, pd, nrpd, rpar, ipar)
    ...
    real(kind=rt), intent(IN   ) :: y(neq), rpar, t
    real(kind=rt), intent(  OUT) :: pd(neq,neq)
  ...
  subroutine f_rhs_instantaneous_reaction_rates(n, t, y, ydot, rho_Hnuc, rpar, ipar)
    ...
    real(kind=rt) :: dens, temp, T9, T9a
    real(kind=rt) :: rhoh, rho_Hnuc

    real(kind=rt) :: rate
    real(kind=rt) :: sc1212, dsc1212dt
    real(kind=rt) :: xc12tmp

    real(kind=rt), PARAMETER :: &
         one_twelvth = 1.0d0/12.0d0, &
         five_sixths = 5.0d0/ 6.0d0, &
         one_third = 1.0d0/ 3.0d0, &
         two_thirds = 2.0d0/ 3.0d0

    real(kind=rt) :: scratch

    real(kind=rt) :: a, b

    integer, save :: ic12, io16, img24

    real(kind=rt) :: X(nspec)
