...
contains
  ...
  subroutine actual_burner(rhoXin, rhohin, dt, rhoout, rhoXout, rhohout, &
    ...
    real(kind=rt), intent(in   ) :: rhoXin(nspec), rhohin, dt
    real(kind=rt), intent(  out) :: rhoout, rhoXout(nspec), rhohout
    real(kind=rt), intent(in   ) :: sdc_rhoX(nspec), sdc_rhoh
    real(kind=rt), intent(in   ) :: p0
    ...
    real(kind=rt), dimension(NEQ) :: atol, rtol
    ...
    real(kind=rt), dimension(LRW) :: rwork
