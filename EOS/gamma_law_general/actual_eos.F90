...
contains
  ...
  subroutine actual_eos(input, state, state_comp)
    ...
    case (eos_input_rh)
       ...
       state % T = (state % h * state_comp % mu * m_nucleon / k_B)*(gamma_const - ONE)/gamma_const
    ...
    case (eos_input_tp)
       ...
       state % rho =  state % p * state_comp % mu * m_nucleon / (k_B * state % T)
    ...
    case (eos_input_rp)
       ...
       state % T = state % p * state_comp % mu * m_nucleon / (k_B * state % rho)
    ...
    case (eos_input_re)
       ...
       state % T   = state % e * state_comp % mu * m_nucleon * (gamma_const-ONE) / k_B
    ...
    case (eos_input_ps)
       ...
       state % T  = state % p**(TWO/FIVE) * &
                    ( TWO*M_PI*hbar*hbar/(state_comp % mu * m_nucleon) )**(THREE/FIVE) * &
                    dexp(TWO * state_comp % mu * m_nucleon*state % s/(FIVE*k_B) - ONE) / k_B
    ...
    state % p = state % rho * state % T * (k_B / (state_comp % mu * m_nucleon))
    ...
    state % s = (k_B/(state_comp % mu * m_nucleon))*(2.5_rt + &
         log( ( (state_comp % mu * m_nucleon)**2.5_rt * rhoinv )*(k_B * state % T)**1.5_rt * fac ) )
    ...
    state % dedr = ZERO
    state % dsdT = 1.5_rt * (k_B / (state_comp % mu * m_nucleon)) * Tinv
    state % dsdr = - (k_B / (state_comp % mu * m_nucleon)) * rhoinv
...
#ifdef EXTRA_THERMO
    state % dpdA = - state % p * (ONE/state_comp % abar)
    state % dedA = - state % e * (ONE/state_comp % abar)

    if (assume_neutral) then
      state % dpdZ = ZERO
      state % dedZ = ZERO
    else
      state % dpdZ = state % p * (ONE/(ONE + state_comp % zbar))
      state % dedZ = state % e * (ONE/(ONE + state_comp % zbar))
    endif
#endif
